...
2. 小标题不会自动办理 warp 并且生成 id 和 hash 链接，要用 `$Section.id`
...
1. zine 采用 id Slot 来 Slot, 灵活性比普通的 Slot 要强一些，可以分开 head Slot 和 content Slot, 这样就很方便资源引用等操作的细粒度控制。
...
2. scripty 的语法有点 funtional programming 的思想。
