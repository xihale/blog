...
这就意味着，即使某个无限期存活的闭包完全没有捕获任何变量，整个包含了其他占用大量内存的变量的共享 `Context` 也会随之常驻内存中，无法被垃圾回收（GC）。只要该闭包没有被释放，整个 Context 及其内部的**所有引用**（哪怕是其他已经消亡的闭包所捕获的庞大对象）就会被强制保留。在某些情况下（如 DOM 事件监听、全局定时器、长连接回调等闭包声明可能无限长生命周期的场景），这会造成极其隐蔽但极为严重的内存泄漏。
